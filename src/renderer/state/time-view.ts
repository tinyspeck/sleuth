import { getUnixTime, startOfMinute, getMinutes, add, set, startOfHour } from 'date-fns';
...
function getBucket(range: number, momentValue: number): number {
    if (range < 1000 * 60 * 60 * 4) {
      ...
      return getUnixTime(set(add(momentValue, {minutes: r}), {seconds: 0, milliseconds: 0}));
 ...
 */
 export function getTimeBuckedLogMetrics(selectedLogFile: SelectableLogFile, range: number): TimeBucketedLogMetrics {
   if (!isLogFile(selectedLogFile)) {
    ...
    for (const entry of selectedLogFile.logEntries) {
      if (entry.momentValue) {
        ...
        values[bucket] = values[bucket] || { info: 0, warn: 0, error: 0, debug: 0 };
