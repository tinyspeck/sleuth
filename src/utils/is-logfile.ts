...
export function isProcessedLogFile(
  ...
  if (file && (file as ProcessedLogFile).type) {
    return (
      (file as ProcessedLogFile).type &&
      (file as ProcessedLogFile).type === 'ProcessedLogFile'
    );
  }
...
export function isMergedLogFile(
  ...
  if (file && (file as MergedLogFile).type) {
    return (
      (file as MergedLogFile).type &&
      (file as MergedLogFile).type === 'MergedLogFile'
    );
  }
...
export function isUnzippedFile(file?: SelectableLogFile): file is UnzippedFile {
  if (file && (file as UnzippedFile).fullPath) {
    return !!(
      (file as UnzippedFile).fileName !== undefined &&
      (file as UnzippedFile).fullPath !== undefined
    );
  }
